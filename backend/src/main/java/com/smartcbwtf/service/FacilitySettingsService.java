...
public class FacilitySettingsService {
    ...
    public void updateLegalProfile(LegalProfileDTO dto, String ipAddress) {
        ...
        auditIfChanged("legal", "authorizationNumber", settings.getAuthorizationNumber(), dto.authorizationNumber(),
        ...
        auditIfChanged("legal", "registeredAddress", settings.getRegisteredAddress(), dto.registeredAddress(),
                facilityId, ipAddress);
        auditIfChanged("legal", "registeredState", settings.getRegisteredState(), dto.registeredState(), facilityId,
                ipAddress);
        ...
        auditIfChanged("legal", "signatureUrl", settings.getSignatureUrl(), dto.signatureUrl(), facilityId, ipAddress);
    ...
    public void updateFinancialSettings(FinancialSettingsDTO dto, String ipAddress) {
        ...
        if (settings.isGstLocked()) {
            if (!Objects.equals(settings.getCgstPercent(), dto.cgstPercent()) ||
                ...
                throw new SettingsLockedException("GST rates are locked after first invoice generation");
            }
        }

        auditIfChanged("financial", "cgstPercent", str(settings.getCgstPercent()), str(dto.cgstPercent()), facilityId,
                ipAddress);
        auditIfChanged("financial", "sgstPercent", str(settings.getSgstPercent()), str(dto.sgstPercent()), facilityId,
                ipAddress);
        auditIfChanged("financial", "igstPercent", str(settings.getIgstPercent()), str(dto.igstPercent()), facilityId,
                ipAddress);
        auditIfChanged("financial", "gstEnabled", str(settings.getGstEnabled()), str(dto.gstEnabled()), facilityId,
                ipAddress);
    ...
    public void updatePaymentReminders(PaymentReminderDTO dto, String ipAddress) {
        ...
        auditIfChanged("payment", "gracePeriodDays", str(settings.getGracePeriodDays()), str(dto.gracePeriodDays()),
                facilityId, ipAddress);
    ...
    public void updateAgreementRules(AgreementRulesDTO dto, String ipAddress) {
        ...
        auditIfChanged("agreement", "defaultAgreementValidityMonths", str(settings.getDefaultAgreementValidityMonths()),
    ...
    public void updateOperationalRules(OperationalRulesDTO dto, String ipAddress) {
        ...
        auditIfChanged("operational", "qrValidityDays", str(settings.getQrValidityDays()), str(dto.qrValidityDays()),
                facilityId, ipAddress);
    ...
    public void updateComplianceSettings(ComplianceSettingsDTO dto, String ipAddress) {
        ...
        auditIfChanged("compliance", "dailyReportTime", str(settings.getDailyReportTime()), str(dto.dailyReportTime()),
                facilityId, ipAddress);
        ...
        auditIfChanged("compliance", "enforceChecksum", str(settings.getEnforceChecksum()), str(dto.enforceChecksum()),
                facilityId, ipAddress);
    ...
    public void updateEmailSettings(EmailSettingsDTO dto, String ipAddress) {
        ...
        auditIfChanged("email", "senderName", settings.getSenderName(), dto.senderName(), facilityId, ipAddress);
        auditIfChanged("email", "senderEmail", settings.getSenderEmail(), dto.senderEmail(), facilityId, ipAddress);
        ...
        settings.setSenderName(dto.senderName());
        settings.setSenderEmail(dto.senderEmail());
    ...
    public Page<SettingsAuditDTO> getAuditHistory(String section, int page, int size) {
        ...
        if (section != null && !section.isBlank()) {
            logs = auditLogRepository.findByFacilityIdAndSectionOrderByChangedAtDesc(facilityId, section, pageable);
        } else {
    ...
    private FacilitySettingsDTO toDTO(FacilitySettings s) {
        return new FacilitySettingsDTO(
                ...
                new LegalProfileDTO(
                        ...
                        s.getRegisteredAddress(), s.getRegisteredState(), s.getRegisteredPincode(),
                        s.getOfficialEmail(), s.getOfficialPhone(),
                        s.getLogoUrl(), s.getLogoChecksum(), s.getSignatureUrl(), s.getSignatureChecksum()),
                new FinancialSettingsDTO(
                        s.getCgstPercent(), s.getSgstPercent(), s.getIgstPercent(), s.getGstEnabled()),
                new PaymentReminderDTO(s.getGracePeriodDays(), s.getAutoAlertEscalation()),
                new AgreementRulesDTO(
                        s.getDefaultAgreementValidityMonths(), s.getAgreementRenewalWindowDays(),
                        s.getBlockOverlappingAgreements()),
                new OperationalRulesDTO(
                        s.getQrValidityDays(), s.getAllowMultipleActiveQrs(), s.getRequireCbwtfVerification(),
                        s.getGpsGeofenceRadiusM(), s.getMaxUnverifiedBags(), s.getBlueWasteMinPercent()),
                new ComplianceSettingsDTO(
                        s.getDailyReportTime(), s.getMonthlyReportDay(), s.getAnnualFormIvDate(),
                        s.getEnforceChecksum()),
                new EmailSettingsDTO(
                        s.getSenderName(), s.getSenderEmail(), s.getCcAdminOnHcfEmails(),
                        s.getEmailNotificationsEnabled(), s.getInAppAlertsEnabled()),
                new FacilitySettingsDTO.LockedFieldsDTO(
                        s.isGstLocked(), s.isComplianceLocked(), s.getFirstQrGeneratedAt() != null,
                        s.getFirstInvoiceAt(), s.getFirstQrGeneratedAt(), s.getFirstComplianceReportAt()),
