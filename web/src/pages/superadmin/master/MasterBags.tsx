...
import { Refresh as RefreshIcon, Inventory as BagIcon } from '@mui/icons-material';
import { DataGrid } from '@mui/x-data-grid';
import type { GridColDef } from '@mui/x-data-grid';
import { adminApi } from '../../../api/admin';

export default function MasterBags() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['master-bags', status, category, page, pageSize],
    queryFn: () => adminApi.listMasterBags({ status: status || undefined, category: category || undefined, page, size: pageSize }),
  });
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" fontWeight={700}>Waste Bags</Typography>
        <Typography variant="body2" color="text.secondary">All bag labels across all CBWTFs (Read-Only)</Typography>
      </Box>
      ...
      {error && <Alert severity="error" sx={{ mb: 3 }}>Failed to load bags.</Alert>}
              ...
              slots={{
                noRowsOverlay: () => (
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', py: 6 }}>
                    <BagIcon sx={{ fontSize: 48, color: 'text.disabled', mb: 1 }} />
                    <Typography color="text.secondary">No bags found</Typography>
                  </Box>
                ),
              }}
            />
