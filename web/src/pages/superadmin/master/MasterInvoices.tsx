...
import { Refresh as RefreshIcon, Receipt as InvoiceIcon } from '@mui/icons-material';
...
export default function MasterInvoices() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['master-invoices', status, page, pageSize],
    queryFn: () => adminApi.listMasterInvoices({ status: status || undefined, page, size: pageSize }),
  });
  ...
  const columns: GridColDef[] = [
    ...
    { 
      ...
      headerAlign: 'right',
      align: 'right',
      renderCell: (p) => p.value != null ? `₹${Number(p.value).toLocaleString()}` : '-',
    },
    { 
      field: 'status', 
      headerName: 'Status', 
      ...
      renderCell: (p) => (
        <Chip 
          label={p.value || '-'} 
          size="small" 
          color={p.value === 'PAID' ? 'success' : p.value === 'SENT' ? 'primary' : p.value === 'DRAFT' ? 'default' : 'error'} 
        />
      ),
    },
    { 
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">All invoices across all CBWTFs (Read-Only)</Typography>
      </Box>
      ...
      {error && <Alert severity="error" sx={{ mb: 3 }}>Failed to load invoices.</Alert>}
              ...
              slots={{
                noRowsOverlay: () => (
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', py: 6 }}>
                    <InvoiceIcon sx={{ fontSize: 48, color: 'text.disabled', mb: 1 }} />
